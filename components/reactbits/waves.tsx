...
/**
 * Vendored from React Bits (https://reactbits.dev) — Waves, TypeScript +
...
const Waves: React.FC<WavesProps> = ({
  ...
  // MedBell edit: upstream calls Math.random() during render and allocates a
  // Noise on every render, since useRef evaluates its argument each time and
  // then throws the result away. Seeded once, inside the effect, instead.
  ...
  useEffect(() => {
    ...
    // MedBell edit: a continuously animating backdrop is exactly what
    // prefers-reduced-motion is for. Draw one static frame, keep resize
    // working so it stays crisp, and never start the loop or track the cursor.
    const reduceMotion =
  ...
  return (
    ...
    >
      {/* MedBell edit: the upstream cursor dot is a hardcoded #160000, which
          reads as a stray black speck on our surfaces. Removed. */}
